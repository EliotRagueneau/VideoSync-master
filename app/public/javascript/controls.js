...
function preview_play() {
    ...
    if (videoT.currentTime === 0 && videoGP.currentTime === 0) {
        let sync_time_T = localStorage.getItem('sync_frame_T') * localStorage.getItem('frameTimeT');
        let sync_time_GP = localStorage.getItem('sync_frame_GP') * localStorage.getItem('frameTimeGP');
...
function reset() {
    let txt;
    let r = confirm("Are you sure to go home?\nYour work will be discarded...");
    if (r === true) {
        localStorage.clear();
        location.href = '/';
    }
...
function save() {
    ...
    let frameRateT = 1 / localStorage.getItem('frameTimeT');
    ...
    let xmltext = "<informations>" +
        "<videoID>" + localStorage.getItem('videoID') + "</videoID>" +
        "<frame_rate_G>" + frameRateT + "</frame_rate_G>" +
        "<synch_frame_G>" + localStorage.getItem('sync_frame_T') + "</synch_frame_G>" +
        "<start_frame_G>" + start_frame_G + "</start_frame_G>" +
        "<end_frame_G>" + localStorage.getItem('end_frame_T') + "</end_frame_G>" +
        "<frame_rate_L>" + frameRateGP + "</frame_rate_L>" +
        "<synch_frame_L>" + localStorage.getItem('sync_frame_GP') + "</synch_frame_L>" +
        "<start_frame_L>" + start_frame_L + "</start_frame_L>" +
        "<end_frame_L>" + localStorage.getItem('end_frame_GP') + "</end_frame_L>" +
        "<frame_rate_R>" + frameRateGP + "</frame_rate_R>" +
        "<synch_frame_R>" + synch_frame_R + "</synch_frame_R>" +
        "<start_frame_R>" + start_frame_R + "</start_frame_R>" +
        "<end_frame_R>" + end_frame_R + "</end_frame_R>" +
        "</informations>";
