...
InfoVideo::InfoVideo(const char *fileName) {
    _fileName = fileName;
    _delay = 0;
    _frameRate = 0;
}
...
//    InfoVideo test("./public/uploads/moncul.txt");
//    test.parse("Mon cul Test");
//
//    int hr, hl, mr, ml, sr, sl, fr, fl;
//    hr = std::stoi(test.getTimeCode().substr(0, 2));
//    hl = std::stoi(test.getTimeCode().substr(0, 2));
//    mr = std::stoi(test.getTimeCode().substr(3, 5));
//    ml = std::stoi(test.getTimeCode().substr(3, 5));
//    sr = std::stoi(test.getTimeCode().substr(6, 8));
//    sl = std::stoi(test.getTimeCode().substr(6, 8));
//    fr = std::stoi(test.getTimeCode().substr(9, 11));
//    fl = std::stoi(test.getTimeCode().substr(9, 11));
//
//    float f1;
//    f1 = fl / test.getFrameRate() - fr / test.getFrameRate();
//    int timeDiff = (sr + mr * 60 + hr * 60 * 60) - (sl + ml * 60 + hl * 60 * 60);
//    int frameNbdiff = (sr + mr * 60 + hr * 60 * 60) * test.getFrameRate() + fr -
//                      ((sl + ml * 60 + hl * 60 * 60) *     test.getFrameRate() + fl);
//    float delay = timeDiff + f1;
//    saveData(&test, &test, &test, frameNbdiff);
//
//
//    if (delay < 0)
//        test.setDelay(-delay);
...
//    return delay * 100;
    ...
    if (argc < 3) {
        std::cerr << "Usage : ./parser <R video file> <L video file> <G video file> <G video name>" << '\n';
        return -1;
    }
    ...
    int hr, hl, mr, ml, sr, sl, fr, fl;
    if (!rVideo.getTimeCode().empty() && !lVideo.getTimeCode().empty()) {
    ...
    int timeDiff = (sr + mr * 60 + hr * 60 * 60) - (sl + ml * 60 + hl * 60 * 60);
    int frameNbdiff = (sr + mr * 60 + hr * 60 * 60) * rVideo.getFrameRate() + fr -
                      ((sl + ml * 60 + hl * 60 * 60) * lVideo.getFrameRate() + fl);
    float delay = timeDiff + f1;
